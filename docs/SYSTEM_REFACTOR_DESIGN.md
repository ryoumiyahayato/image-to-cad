...
状态：已批准的实现基线；阶段 1–5 已按独立证据顺序核验，阶段 6–12 尚未验收
...
首个强制 CI 集必须包含至少三份完整页面、至少两个独立源文档，禁止用同一截图的多个裁剪凑数。阶段 4 基线现有 12 次完整页面运行、10 个唯一源页面、3 个独立源文档和 22 个覆盖类别；包含透视相机照片、破损/阴影/折痕工程扫描、多页 PDF、纵向表格/Logo 页面，以及 72–600 DPI 页面，并同时执行 OCR、追踪、预览、结构分析和 DXF 审计。
...
`main.run_gui -> gui_public_release.MainWindow -> gui_final_release -> gui_librecad_release -> optimized_trace.trace_image_optimized -> scan_cleanup -> straight_line_reconstruction -> ContentOwnership -> FinalStructure -> preview/export`

命令行入口固定为：

`main.run_headless -> pipeline.run_pipeline -> PipelineService.vectorize -> preprocess -> detect_lines -> source-coordinate retention -> topology validation only -> classification/export`

两条路径都禁止全页几何改写。产品 GUI 的结构重建只允许在 `StructuralRoiSet` 中提出交点延伸；命令行兼容路径不执行端点移动、共线合并、相交切分或桥接，只保留源检测坐标并做只读拓扑校验。

### 10.2 等价全页运算清单

| 风险类别 | 生产状态 | 守卫或边界 |
| --- | --- | --- |
| `MORPH_CLOSE` | 应用代码为 0 处 | 静态测试扫描整个 `app`；运行时测试记录所有形态学操作 |
| 先膨胀后腐蚀的等价闭运算 | 不用于最终前景 | `perspective` 只处理纸张候选掩膜；其余膨胀只生成保护/候选/删除掩膜，不写入最终前景 |
| 大卷积后阈值连接 | 不存在输出加前景路径 | 高斯模糊只估计背景；`boxFilter` 只统计尘点密度并用于删除 |
| 全页 Hough 跨隙 | 强制关闭 | GUI 优化路径和 `PipelineService` 均将 `maxLineGap/max_line_gap` 固定为 0 |
| 全页端点桥接 | 生产路径禁用 | 通用 `GeometryCleanParams.max_bridge_gap` 即使由旧 UI 传入也在服务边界归零 |
| 全页共线合并 | 生产路径禁用 | `clean_geometry` 与 `collapse_scan_parallel_duplicates` 不在生产结构重建中调用 |
| 全页端点吸附 | 生产路径禁用 | `snap_distance=0`；运行时报告要求首轮/末轮移动端点数均为 0 |
| 全页交点延伸 | 生产路径禁用 | 无显式 `StructuralRoiSet` 或源前景即直接返回；ROI 外候选稳定拒绝为 `outside_structural_roi` |
| 其他大面积新增前景 | 生产路径禁用 | 输出结构线必须通过源支持过滤；唯一允许的新增是 ROI 内、保护掩膜外、连通域判定允许的短延伸 |

`geometry_cleaner.py`、`geometry_normalized.py`、`topology.py` 和
`collapse_scan_parallel_duplicates` 仍作为兼容 API 与隔离单元测试对象保留；它们不是生产调用链的一部分。后续如果重新接入，必须先提供显式 ROI 类型、运行时观测和真实文档差异报告，不能只依赖参数为 0。

### 10.3 四版本真实页面对照

阶段 5 冻结阶段 4 清单中的 10 个唯一页面，OCR 始终开启，并记录输入/源代码 SHA-256。四组为：

- 旧算法基线 `6a2aa545a534ea6fd50db8a5ba942a0a5db9e353`；
- 历史不安全版本 `d9fbda763e95c6dd7154934b801b59dbf034e711`；
- 与当前生产源代码相同、但显式旁路 ROI 延伸的“全页运算关闭版”；
- 当前结构 ROI 生产版本。

历史不安全版本与当前结构 ROI 版本的聚合差异如下：

| 指标 | 历史不安全 | 当前 ROI | 变化 |
| --- | ---: | ---: | ---: |
| 最终新增前景像素 | 2,377,295 | 1,448,727 | -39.06% |
| 合并的源组件 | 8,155 | 6,132 | -24.81% |
| 结构线造成的组件合并 | 1,326 | 247 | -81.37% |
| 正文区域新增像素 | 292,423 | 211,937 | -27.52% |
| 正文连通粘连次数 | 622 | 126 | -79.74% |
| 禁止区域新增像素 | 48,108 | 32,786 | -31.85% |
| 错误桥接次数 | 411 | 70 | -82.97% |
| 涉及错误桥接的禁止区域 | 7 | 5 | -28.57% |
| 覆盖的正确修复区域 | 3 | 3 | 不变 |

“全页运算关闭版”与当前 ROI 版使用完全相同的生产源代码；当前 ROI 只多写入 119 个像素，错误桥接、正文粘连、组件合并和正确修复区域均未增加或减少。它证明当前 ROI 延伸没有重现历史连通问题，但在这 10 页上也没有产生额外连通修复收益。此结果是长期维护风险：在扩大带真值断点的回归集之前，不得扩大 ROI 类型、距离预算或候选范围。

连通性判定的所有掩膜和连通域标签均在单个 ROI 的局部坐标内生成。每个 ROI 的源结构掩膜、保护掩膜和组件标签只构建一次；每条桥只检查自身包围盒内触碰的标签。300 DPI 大表格页的无 OCR 诊断从超过 180 秒仍未完成降至 7.7 秒，600 DPI 页降至 35.1 秒。平行重复检测只允许删除重复表示并保留最长的原始检测坐标；禁止使用区间并集、端点移动或共线桥接减少对象数量。

OCR 摘要和最终对象类型摘要相对旧算法在 10/10 页发生变化；阶段 5 只把它作为版本差异记录，不把 OCR 或对象类别变化解释为连通性安全结论。连通性统计只使用二值掩膜和连通域标签，不修改对象类型。

可复现入口为 `cad_photo_to_dxf/scripts/run_topology_comparison.py`，版本化结果为 `cad_photo_to_dxf/validation/whole-page-topology/phase5-comparison.json`。运行时门禁位于 `tests/test_whole_page_topology_guard.py`，静态系统约束位于 `tests/test_system_refactor_invariants.py`。
