...
from app.draftsman_vs2 import run_draftsman_vs2, write_vs2_artifacts  # noqa: E402


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate the shadow-only Draftsman VS2 electrical package."
    )
...
def main() -> int:
    ...
    if len(set(replay_identities)) != 1:
        raise SystemExit("VS2 replay identities are not deterministic")
    result = results[0]
    if not result.quality.passed:
        raise SystemExit(f"VS2 quality gate failed: {result.quality.to_dict()}")
    paths = write_vs2_artifacts(
        args.output_dir,
