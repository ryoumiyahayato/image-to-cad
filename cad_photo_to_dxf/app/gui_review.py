...
class MainWindow(_GuardedMainWindow):
    """Guarded GUI with direct visual editing on top of the scan."""
    ...
    def review_layers(self) -> None:
        if not self.lines:
            ...
            return
        background = self.corrected_image if self.corrected_image is not None else self.binary_image
        ...
        if changed:
            warning = (
                f"可视化人工修改影响 {changed} 条实体；"
                f"其中删除 {dialog.deleted_count} 条。"
            )
            self._last_warnings = tuple(dict.fromkeys((*self._last_warnings, warning)))
        self.statusBar().showMessage(
            f"可视化修改完成；保留 {len(reviewed)} 条，修改或删除 {changed} 条"
        )

    def export_file(self) -> None:
        # The separate circle-confirmation workflow was removed.  Scan fidelity
        # is preserved by the underlay; reviewed line entities remain editable.
