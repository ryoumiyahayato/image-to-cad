...
默认通过条件是：audit 错误为 0、非法坐标为 0、零长度为 0、重复实体为 0。悬空端点只作为指标，因为轴线、标注线和局部线可能有意开放。
...
没有实际执行 FreeCAD 时，报告不能写成“FreeCAD 已验证”。
...
应用版本只维护在 `app/version.py`。PyInstaller 资源文件在构建时生成，Inno Setup 从已构建 EXE 读取版本。不要手工修改安装包版本字符串。
